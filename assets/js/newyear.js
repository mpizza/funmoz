...
$(function(){
  ...
  $('ul#bg-select li').on('click', function(){
     $('li.bg-check').removeClass('bg-check');
     $(this).addClass('bg-check');
     setBGsrc($('li.bg-check img').attr('data-original'));
  });
  ...
  $('#u_text').bind('keyup',function(){
    var count = chineseCount($('#u_text').val()),
        w_limit = 25;
    $('#word_count').html(count);
    if(count>w_limit){
      $('#word_count').css('color','#fff');
      $('#u_text').val($('#u_text').val().substring(0,w_limit));
      $('#word_count').html(w_limit);
    }
    pubs.valid = false;
    pubs.draw();
  });
  ...
  $('#c_save').on('click', function(){
    ...
    $.post('/xmas/save_canvas/',{dataurl:dataimgurl, u_email:u_email, u_checked:u_check},function(data){
      //$('#test').html(data);
      if(data!=='failed'){
        window.location = '/newyear/cards/'+data+'/';
      }
    });
